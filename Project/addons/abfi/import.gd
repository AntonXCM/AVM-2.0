...
func _import(
	...
	for i in ranges_n:
		...
		if len >= 3:
			var dash := r.find("-")
			if dash == -1:
				push_error("range %d must contain '-'" % i)
				return ERR_PARSE_ERROR

			var start_str := r.substr(0, dash)
			var end_str := r.substr(dash + 1)

			var start := _parse_code(start_str)
			var end := _parse_code(end_str)

			if start < 0 or end < 0:
				push_error("range %d has invalid character code" % i)
				return ERR_PARSE_ERROR
			if start > end:
				push_error("range %d's start character code is greater than its end" % i)
				return ERR_PARSE_ERROR
			if start == end:
				push_error("range %d's start and end characters are identical" % i)
				return ERR_PARSE_ERROR
